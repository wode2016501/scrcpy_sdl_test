...
ubuntu: 
shell: c_h264_sdl_9999.c -lavcodec -lavformat -lavutil -lswscale -lSDL2
shell: ./a.out
*/
// gcc c_h264_sdl_9999.c  -lavcodec -lavformat -lavutil -lswscale -lSDL2
//  ./a.out
#include <stdio.h>
#include <SDL2/SDL.h>
#include <stdio.h>
...
int main(int argc, char *argv[])
{
    ...
    int fd = tcp_connect("192.168.100.6", 9999);
    if (fd < 0)
    {
        printf("无法打开文件: %s\n", H264_FILE);
        return -1;
    ...
    int ret = read_(fd, buffer, 77,size_max);
    if (ret != 77)
    {
        printf("读取77失败\n");
        return -1;
    ...
    ret = read_(fd, buffer, 8, size_max);
    if (ret != 8)
    {
        printf("读8失败\n");
        close(fd);
    ...
    buffer_size = read_(fd, &isize, 4, size_max); // size int
    if (buffer_size != 4)
    {
        printf("读4失败\n");
        close(fd);
    ...
    buffer_size = read_(fd, buffer, isize, size_max);
    ...
    */
    ...
    while (!quit)
    {
        // 读取H264数据包
        // ret = lseek(fd, 8, SEEK_CUR);     // pts long
        ret = read_(fd, p, 8, size_max);
        if (ret != 8)
        {
            printf("读8失败\n");
            break;
        }
        buffer_size = read_(fd, &size, 4, size_max); // size int
        if (buffer_size != 4)
        {
            printf("读4失败\n");
            break;
        ...
        printf("size:%d\n", size);
        // 读取一帧数据（简化处理，实际应该按NALU单元读取）
        buffer_size = read_(fd, p, size, size_max);
        if (buffer_size != size)
        {
            printf("文件读取失败\n");
            break;
